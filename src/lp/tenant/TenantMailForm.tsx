...
export default function TenantMailForm() {
  return (
    <section style={{ padding: "64px 24px", background: "#f9f8f6" }}>
      ...
      <div style={{ maxWidth: "720px", margin: "0 auto" }}>
        ...
        <form
          ...
          action="https://formsubmit.co/d.roots001@gmail.com"
          method="POST"
        >
          <input type="hidden" name="_subject" value="【テナントLP】物件情報のお問い合わせ" />
          <input type="hidden" name="_captcha" value="false" />
          <input type="hidden" name="_next" value="" />

          <label style={{ fontSize: "12px", fontWeight: 500, color: "#555", display: "flex", flexDirection: "column", gap: "6px" }}>
            お名前
            <input type="text" name="name" placeholder="山田 太郎" required className="t-mail-input" style={inputStyle} />
          </label>
          <label style={{ fontSize: "12px", fontWeight: 500, color: "#555", display: "flex", flexDirection: "column", gap: "6px" }}>
            メールアドレス
            <input type="email" name="email" placeholder="example@email.com" required className="t-mail-input" style={inputStyle} />
          </label>
          <label style={{ fontSize: "12px", fontWeight: 500, color: "#555", display: "flex", flexDirection: "column", gap: "6px" }}>
            希望エリア・業態など
            <textarea
              name="message"
              placeholder="例）仙台駅周辺で居酒屋を検討中です。居抜き物件があれば教えてください。"
              className="t-mail-input"
